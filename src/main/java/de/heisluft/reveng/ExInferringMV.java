...
public class ExInferringMV extends MethodVisitor implements Util {
  ...
  public void visitTryCatchBlock(Label start, Label end, Label handler, String type) {
    if(DEBUG) System.out.println("TryCatch(start: " + start + ", end: " + end + ", handler: " + handler + ", exType: " + type);
  ...
  public void visitInsn(int opcode) {
    ...
    if(opcode == IMUL || opcode == IADD || opcode == IREM || opcode == IDIV || opcode == ISUB || opcode == IAND ||
        opcode == IOR || opcode == IXOR || opcode == ISHL || opcode == ISHR || opcode == IUSHR) {
      stack.pop();
      stack.pop();
      stack.push("I");
    }
    if(opcode == LMUL || opcode == LADD || opcode == LREM || opcode == LDIV || opcode == LSUB || opcode == LAND ||
        opcode == LOR || opcode == LXOR || opcode == LSHL || opcode == LSHR || opcode == LUSHR) {
      stack.pop();
      stack.pop();
      stack.push("J");
    }
    ...
    if(opcode == ATHROW) {
      ...
      if(exType != null && isSignificant(exType, caughtExceptions)) thrownExTypes.add(exType);
    }
    if(DEBUG) {
      if (currentLabel != null) System.out.print("  ");
  ...
  public void visitIntInsn(int opcode, int operand) {
    ...
    if(opcode == NEWARRAY) {
      stack.pop();
      switch (operand) {
        case T_BOOLEAN: stack.push("[Z"); break;
        case T_CHAR: stack.push("[C"); break;
        case T_BYTE: stack.push("[B"); break;
        case T_SHORT: stack.push("[S"); break;
        case T_INT: stack.push("[I"); break;
        case T_LONG: stack.push("[J"); break;
        case T_DOUBLE: stack.push("[D"); break;
        case T_FLOAT: stack.push("[F"); break;
      }
    }
    if(DEBUG) {
      if (currentLabel != null) System.out.print("  ");
  ...
  public void visitEnd() {
    if(currentLabel != null) System.out.println("}");
    currentLabel = null;
    stack.clear();
    locals.clear();
    if(!thrownExTypes.isEmpty()) System.out.println("throws " + thrownExTypes);;
    System.out.println();
    super.visitEnd();
  }
  ...
  boolean isSignificant(String exType, List<String> caughtExceptions) {
    if(caughtExceptions.stream().map(this::desc).anyMatch(exType::equals)) return false;
    Class<?> jExType = resolveClass(desc(exType));
    ...
    for (String cEx : caughtExceptions) {
      Class<?> jEx = resolveClass(desc(cEx));
  ...
  String localsToString() {
    return "locals: " + Tuple2.streamMap(locals).sorted(Comparator.comparing(Tuple2::_1)).map(Tuple2::_2).collect(Collectors.joining(", ", "{ ", " }"));
  }

  private static Class<?> resolveClass(String desc) {
  ...
  private static Executable resolveMethod(Class<?> c, String name, String desc, Type[] argTypes) {
    ...
    try {
      Executable ex;
      if("<init>".equals(name)) {
        ex = c.getConstructor(jArgTypes);
      } else {
        ex = c.getMethod(name, jArgTypes);
      }
      ...
      try {
        Executable ex;
        if ("<init>".equals(name)) {
          ex = c.getDeclaredConstructor(jArgTypes);
        } else {
          ex = c.getDeclaredMethod(name, jArgTypes);
        }
        methodCache.put(c + "#" + name + desc, ex);
        return ex;
